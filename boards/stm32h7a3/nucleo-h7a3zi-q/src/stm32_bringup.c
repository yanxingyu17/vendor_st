/****************************************************************************
 * boards/arm/stm32h7/nucleo-h7a3zi-q/src/stm32_bringup.c
 *
...
int stm32_bringup(void)
{
  ...
  /* Generic lower-half BUTTON driver. Requires INPUT_BUTTONS_LOWER +
   * INPUT_BUTTONS + ARCH_BUTTONS + ARCH_IRQBUTTONS. H7A3ZI-Q has only
   * the USER button on PC13; IRQ-button support is a follow-up.
   */
...
#ifdef CONFIG_ONESHOT
  os = oneshot_initialize(2, 10);
  if (os != NULL)
    {
      ret = oneshot_register("/dev/oneshot/0", os);
      if (ret < 0)
        {
    ...
    {
      syslog(LOG_ERR, "ERROR: oneshot_initialize(TIM2) failed\n");
    }
#endif
